<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="挂号费用" prop="guahaofeiyong">
          <el-input v-model="ruleForm.guahaofeiyong" 
              placeholder="挂号费用" clearable  :readonly="ro.guahaofeiyong"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="挂号费用" prop="guahaofeiyong">
              <el-input v-model="ruleForm.guahaofeiyong" 
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/paibanleixing/banci`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.banciOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    gonghaoChange () {
      this.$http({
        url: `follow/yihu/gonghao?columnValue=`+ this.ruleForm.gonghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.gerenjianjie = this.ruleForm.gerenjianjie.replace(reg,'../../../springcloudjf5zc/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
